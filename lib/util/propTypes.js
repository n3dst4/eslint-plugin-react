...
module.exports = function propTypesInstructions(context, components, utils) {
  ...
  class DeclarePropTypesForTSTypeAnnotation {
    ...
    searchDeclarationByName(node) {
      ...
      if (astUtil.isTSTypeReference(node)) {
        ...
        if (shouldTraverseTypeParams && typeArguments && typeArguments.length !== 0) {
          ...
          const nextNode = typeArguments.params[idx];
    ...
    convertReturnTypeToPropTypes(node) {
      ...
      if (typeArguments) {
        if (typeArguments.params.length === 1) {
          let returnType = typeArguments.params[0];
          // This line is trying to handle typescript-eslint-parser
            ...
            if (Array.isArray(returnTypeFunction)) {
              ...
              returnTypeFunction.forEach((func) => {
                if (isFunctionType(func)) {
                  ...
                  const resTypeArguments = res.typeArguments || res.typeParameters;
                  switch (res.type) {
                    case 'ObjectExpression':
                      iterateProperties(context, res.properties, (key, value, propNode) => {
                        if (propNode && propNode.argument && propNode.argument.type === 'CallExpression') {
                          const propNodeTypeArguments = propNode.argument.typeArguments
                    ...
                    case 'CallExpression':
                      if (resTypeArguments) {
                        this.visitTSNode(resTypeArguments);
                      } else {
                        // Ignore this CallExpression return value since it doesn't have any typeArguments to let us know it's types.
                        this.shouldIgnorePropTypes = true;
                      }
      ...
      case 'GenericTypeAnnotation':
        if (propTypes.id.name === '$ReadOnly') {
          ignorePropsValidation = declarePropTypesForObjectTypeAnnotation(
            (propTypes.typeArguments || propTypes.typeParameters).params[0],
            declaredPropTypes
          );
        } else {
          ignorePropsValidation = true;
  ...
  function markAnnotatedFunctionArgumentsAsDeclared(node) {
    ...
    if (
      ...
      const typeArguments = node.parent.typeArguments || node.parent.typeParameters;
      if (typeArguments
        ...
        const propTypes = typeArguments.params[1];
